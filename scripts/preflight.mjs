...
for (const needle of ["ai-tools-workbench.html", "template-library.html", "industry-packs.html", "packs/ai-tools-template-pack.html", "solutions/ai-prompt-template-pack.html", "licenses/agency-content-template-license.html", "licenses/team-content-template-library.html", "团队与商业授权", "paid-template-pack.html", "purchase-guide.html", "business-license.html", "business-license-vs-template-pack.html", "businessLicenseCta", "checkout-config.json", "purchaseIntentUrl", "waitlistCopy", "title-factory-starter-pack.html", "data-mode=\"calendar\"", "exportMarkdown"]) {
...
for (const needle of ["买哪个版本", "版本选择助手", "¥99 完整模板包", "¥999 商业授权", "business-license-vs-template-pack.html", "purchaseIntentUrl", "checkout-config.json", "WebPage"]) {
...
for (const needle of ["团队内容模板库授权", "团队商业授权", "普通模板包", "business-license.html"]) {
...
for (const needle of ["AI 工具内容模板包", "Product", "¥99 完整模板包", "paid-template-pack.html"]) {
...
for (const needle of ["AI 提示词模板包", "模板解决方案", "查看完整付费包", "Product"]) {
...
for (const needle of ["数字产品销售页模板", "适合直接找模板的人", "查看付费包页面", "paid-template-pack.html"]) {
...
const publicHtmlFiles = [
  ...
  "business-license-vs-template-pack.html",
  "downloads/title-factory-starter-pack.html",
  ...tools.map((file) => `tools/${file}`),
  ...packs.map((file) => `packs/${file}`),
  ...solutions.map((file) => `solutions/${file}`),
  ...licenses.map((file) => `licenses/${file}`),
];
...
for (const file of publicHtmlFiles) {
  ...
  for (const phrase of internalPublicPhrases) {
    if (html.includes(phrase)) fail(`public page avoids internal phrase: ${file}`, phrase);
  }
}
if (!checks.some((check) => check.name.startsWith("public page avoids internal phrase") && !check.ok)) {
  pass("public pages avoid internal strategy phrases");
}
